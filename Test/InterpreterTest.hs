...
functionClosureCreationTests :: TestTree
functionClosureCreationTests =
  testGroup
    "Function Closure Creation"
    [ timedAssertEqual
        1
        "A closure for single identity function"
        []
        ( Runtime
            ( ( singletonSymbolTable
                  ( Binding
                      (BindingKey "x")
                      []
                      (attachToLamprey ([(treeSU Return . dataNum) 25.0]))
                  )
              ) :
              runtimeSymbolTableStack identityCaller1
            )
            [(treeSU Return . dataId) "x"]
            Nothing
        )
        (createFunctionClosure identityCaller1 identityBinding)
    ]
  where
    ...
    identityBinding =
      Binding
        (BindingKey "id")
        [(makeSU Send . dataId) "x"]
        (attachToLamprey [(treeSU Send . dataId) "x", (treeSU Return . dataId) "x"])
    identityCaller1 :: Runtime
    identityCaller1 =
      Runtime
        [singletonSymbolTable identityBinding]
        [(treeSU Return . dataId) "id" -<= [(treeSU Send . dataNum) 25.0]]
        Nothing
